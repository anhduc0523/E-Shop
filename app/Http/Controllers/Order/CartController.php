...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $already_cart = Cart::where('user_id', auth()->user()->id)->where('order_id', null)->where('product_id', $product->id)->where('size', null)->first();
        if ($already_cart) {
    ...
    public function cartUpdate(Request $request)
    {
        ...
        if ($request->quant && $request->size) {
            ...
            foreach ($request->size as $k => $size) {
                $id = $request->qty_id[$k];
                $cart = Cart::find($id);
                $cart->size = $size;
                $cart->save();
